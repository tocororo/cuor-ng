...
import { PromptComponent } from "./prompt-component.component";
...
export class HomeComponent implements OnInit {
  ...
  public ngOnInit(): void {
    this.harvesterInfo = [
      {
        label: "ONEI",
        icon: "assets/images/logo_onei.jpg",
        text: "CARD_ITEM_INFO_HARV_INFO_TEXT_1",
      },
      {
        label: "GRID",
        icon: "assets/images/grid.jpg",
        text: "CARD_ITEM_INFO_HARV_INFO_TEXT_2",
      },
      {
        label: "Wikidata",
        icon: "assets/images/wikidatawiki.png",
        text: "CARD_ITEM_INFO_HARV_INFO_TEXT_3",
      },
    ];
    ...
    this._cuorService.getOrganizations(null).subscribe({
      next: (searchResponse: SearchResponse<Organization>) => {
        ...
        this.cubanOrganizations = searchResponse.hits.hits.map(
          (hit) => hit.metadata
        );
        console.log(
          "orgs===",
          searchResponse.hits.hits.map((hit) => hit.metadata)
        );
        ...
        this.homeCharts.total = [
          {
            ...
            value: searchResponse.hits.total - this.cubanOrganizationTotal,
          },
